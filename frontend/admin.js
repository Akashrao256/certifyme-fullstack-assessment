...
function showDashboard(admin) {
  ...
  const email = admin?.email?.trim() || "admin@qf.org.qa";
...
async function handleLogout() {
  try {
    ...
    // Even if the backend session is already gone, reset the local UI state.
  }
...
function normalizeOpportunity(opportunity) {
  ...
  return {
    ...opportunity,
    skills,
    applicantsText: opportunity.max_applicants
    ...
    <div class="opportunity-card">
      <div class="opportunity-card-header">
        ...
        <div class="opportunity-meta">
          <span><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>${escapeHtml(item.duration)}</span>
          <span><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>${escapeHtml(item.start_date)}</span>
      ...
      <div class="opportunity-footer">
        ...
        <button class="view-course-btn" style="width: auto; padding: 8px 16px;" data-opportunity-id="${item.id}">
          View Details
        </button>
      </div>
    </div>
...
function renderOpportunities(opportunities) {
    ...
    grid.innerHTML = `
      <div class="opportunity-card">
        ...
        <p class="opportunity-description">
          Create your first opportunity to see it here.
        </p>
      </div>
    `;
    return;
  ...
  grid.innerHTML = opportunities.map(renderOpportunityCard).join("");

  opportunities.forEach((opportunity) => {
    const button = grid.querySelector(
      `[data-opportunity-id="${String(opportunity.id)}"]`,
    );
    if (!button) return;
    ...
    button.addEventListener("click", function () {
      ...
      openOpportunityDetails(item.name, {
        ...
        futureOpportunities: item.future_opportunities,
        prerequisites: item.category,
      });
    });
  });
}
...
document.getElementById("forgotForm").addEventListener("submit", async function (e) {
  ...
  try {
    ...
    showToast(data.message || "Password reset request submitted.");
    generateCaptcha("forgot");
    this.reset();
  } catch (error) {
    showToast(error.message);
